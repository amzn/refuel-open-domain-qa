...
class QAPairEvaluation(object):
    ...
    def print_all_metrics(self, verbose=True):
        all_results = {}
        for metric in self.metrics:
            ...
            if metric=="F1 answer":
                if verbose:
                    print ("%s\t%.2f (all)\t%.2f (multi only)" % (metric, np.mean(result)*100, np.mean(result_multi_only)*100))
                all_results['F1 answer'] = np.mean(result) * 100
                all_results['F1 answer (multi)'] = np.mean(result_multi_only) * 100
            else:
                if verbose:
                    print ("%s\t%.2f" % (metric, np.mean(result_multi_only)*100))
                if metric in ["F1 bleu4", "F1 edit-f1"]:
                    all_results[metric] = np.mean(result_multi_only) * 100
        return all_results
...
def load_prediction(prediction_path, ids):
    if os.path.exists(prediction_path):
        ...
        if "predictions" in prediction.keys():

...
verifier_ckpt = '09082132-MFID-AQ-NoAmb-b64psg100gd8-disa0-e10-lr5e5'
...
for aq_ckpt in aq_ckpts:
    with open('/home/ubuntu/data/MyFusionInDecoderOut/MAP_{}_QD_{}/Verifier_{}/{}_over_generate_pass_1_lm_filtered_prediction.json'.format(aq_ckpt, qd_ckpt, verifier_ckpt, split)) as f:
        model2predictions.append(json.load(f)['th_best'])
...
for id in ids:
    ...
    for model2prediction in model2predictions:
        for pred_qapair in model2prediction[id]:
            ...
            if pred_qapair['answer'] not in answer2question:
                answer2question[pred_qapair['answer']] = {'question': pred_qapair['question'], 'lm_score': pred_qapair['lm_score']}
            else:
                if answer2question[pred_qapair['answer']]['question'] == pred_qapair['question']:
                    continue
                else:
                    if answer2question[pred_qapair['answer']]['lm_score'] <= pred_qapair['lm_score']:
                        continue
                    else:
                        answer2question[pred_qapair['answer']] = {'question': pred_qapair['question'], 'lm_score': pred_qapair['lm_score']}
    ...
    for k, v in curr_answers.items():
        if v >= min_voter:
            id2predictions[id].append({'answer': k, 'question': answer2question[k]['question']})
